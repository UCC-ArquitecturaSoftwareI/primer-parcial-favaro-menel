        ...
        for (auto&[pos, tile] : layer->getTileData()) {
            ...
            if (tile != nullptr) {
               position[i] = {(float) std::get<0>(pos) * ld.getMap().getTileSize().x,
                ...
                int offsetX = tileModX * (ld.getMap().getTileSize().x); //+ Space ) + Margin ; CREO QUE NO TIENE
                int offsetY = currentRow * (ld.getMap().getTileSize().y);// + Space) + Margin; Espacio ni margenes
